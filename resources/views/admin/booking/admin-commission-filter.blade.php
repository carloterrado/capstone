<div id="commissions" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-5xl m-auto">
        ...
        <form method="POST" action="{{ route('commission.history') }}" class="relative bg-white rounded-lg shadow">
            @csrf
            ...
            <div class="">
                <div class="flex justify-between p-4 rounded-t border-b">
                    ...
                    <button before="Close" data-modal-toggle="commissions" type="button"
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6 sm:col-span-3 md:col-span-2 relative">
                            <select name="status"
                                class="cursor-pointer-none block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-900 appearance-none peer">
                                <option selected value="">Filter Commission Status</option>
                                @php
                                    $commissions = [];
                                @endphp
                                @foreach ($histories as $history)
                                    @if (!in_array($history['commission'], $commissions))
                                        <option class="" value="{{ $history['commission'] }}">
                                            {{ $history['commission'] }}</option>
                                        @php
                                            $commissions[] = $history['commission'];
                                        @endphp
                                    @endif
                                @endforeach
                            </select>
                            <label
                                class="pointer-events-none absolute text-sm text-gray-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-gray-900 peer-focus:font-semibold peer-placeholder-shown:scale-100 
                            peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1 whitespace-nowrap">Commission
                                Status</label>
                        </div>
                        <div class="col-span-6 sm:col-span-3 md:col-span-2 relative">
