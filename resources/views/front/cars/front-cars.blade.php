<main id="cars" class="min-h-[calc(100vh-5rem)] md:min-h-[calc(100vh-5.5rem)] grid grid-cols-6 lg:grid-cols-9 gap-6 p-2 py-12 sm:p-6 sm:py-12">
    ...
    <div id="view-car-sort" class="col-span-2 hidden max-w-sm lg:block fixed inset-0  lg:relative overflow-y-auto z-10">
        ...
        <form method="post" action="{{url('cars')}}" name="sortCar" id="sortCar">
            @csrf
            <div class="min-h-screen lg:min-h-full bg-[#f5f5f5] border border-gray-200 rounded-lg shadow-md pt-12 lg:pt-6 p-6 relative">
                ...
                <div class="mt-6">   
                    ...
                    @foreach ($cartypes as $cartype )
                        ...
                        @if ((isset($_POST['type']) && $_POST['type'] === strval($cartype['id'])) || (isset($_GET['type']) && $_GET['type'] === strval($cartype['id'])))
                            selected
                        @endif 
                        >{{$cartype['name']}}</option>
                    @endforeach
                ...
                <div class="mt-6">   
                    ...
                    <option value="25" @if ((isset($_POST['capacity']) && $_POST['capacity'] === '25') || (isset($_GET['capacity']) && $_GET['capacity'] === '25')) selected @endif>25</option>
                    <option value="30" @if ((isset($_POST['capacity']) && $_POST['capacity'] === '30') || (isset($_GET['capacity']) && $_GET['capacity'] === '30')) selected @endif>30</option>
                    </select>
                </div>   
                ...
                <div class="my-6 relative">   
                    ...
                    <div class="container">
                        ...
                        <input name="from" type="range" min="500" max="20000" @if(isset($_POST['from'])) value="{{$_POST['from']}}" @elseif (isset($_GET['from'])) value="{{$_GET['from']}}" @else value="500"  @endif step="100" id="slider-1" oninput="slideOne()">
                        <input name="to" type="range" min="500" max="20000" @if(isset($_POST['to'])) value="{{$_POST['to']}}" @elseif (isset($_GET['to'])) value="{{$_GET['to']}}" @else value="10000"  @endif step="100" id="slider-2" oninput="slideTwo()">
                    </div>
                </div>
